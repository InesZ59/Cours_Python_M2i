...
```bash
Allow car le sid : "LectureDocuments" est Effect Allow
```
...
```bash
deny le s3: DeleteObject n'est pas précisé dans la politique donc il est en défaut
```
...
```bash
Non car il est en défaut donc en statut Deny implicite
```
...
```bash
Non car il y a un deny concernant la partie confidentielle au niveau Get Object qui bloque la politique AmazonS3FullAccess
```
...
```text
Deny t3.micro, il ne fait pas partie de la région. C'est un deny explicite.
aws:RequestedRegion": "eu-west-3",
          "ec2:InstanceType": "t3.micro"
```
3. Il lance une instance `m5.large` dans eu-west-3. Autorisé ou refusé ?
```text
Deny m5.large ne fait pas partie des policies donc deny implicite
Sid": "LancerPetitesInstancesParisSeulement",
      "Effect": "Allow",
```
4. Il exécute `aws ec2 describe-instances --region us-east-1`. Autorisé ou refusé ?
```text
Allow ressources concerne toutes les instances ec2
  "Sid": "LectureEC2Partout",
      "Effect": "Allow",
      "Action": "ec2:Describe*",
      "Resource": "*"
...
```text
Deny : ec2: StopInstances est en mode deny implicite car il n'est pas spécifié dans les policies.
```
...
```text
L'auditeur peut : lire, il ne peut pas créer, et n'est pas noté c'est un deny implicite
"iam:List*",
```
2. L'auditeur peut-il télécharger un objet S3 ? Peut-il en téléverser (`s3:PutObject`) un ?
```text
il peut télécharger l'object S3 car il est Allow 
"Sid": "ReadOnlyExtrait",
      "Effect": "Allow",
      "Action": [
        "ec2:Describe*",
...
```text
le politique gérée peut concerné plusieurs utilisateurs et utilisé plusieurs fois.
```
...
```text
l'action est en mode S3:* qui signifie toutes les actions donc cela inclus 's3:DeleteBucket"
```
...
```text
Les politiques inline sont des politiques qui ne sont pas destinées à être utilisés
le Sid revèle l'utilisation de la policies suite à une dépannage urgent lors de la création de document 
```
```
